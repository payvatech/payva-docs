...
const sheetVariants = cva(
  'fixed z-50 gap-4 bg-primary p-6 focus-visible:outline-none focus-visible:ring-0 focus-visible:ring-transparent focus-visible:ring-offset-0 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500',
  {
    variants: {
      side: {
        ...
        right:
          'inset-y-0 right-0 h-full data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right dark:border-l',
      },
    },
    defaultVariants: {
      side: 'right',
...
const SheetContent = React.forwardRef<React.ElementRef<typeof SheetPrimitive.Content>, SheetContentProps>(
  ({ side = 'right', isMobile, className, children, ...props }, ref) => (
    <SheetPortal>
      <SheetOverlay className={isMobile ? 'xl:hidden' : ''} />
      <SheetPrimitive.Content
        ref={ref}
        className={cn(isMobile ? 'xl:hidden' : '', sheetVariants({ side }), className)}
